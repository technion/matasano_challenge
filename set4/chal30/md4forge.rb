...
def md4forge(string, prependlen, iv)
  # functions
  mask = (1 << 32) - 1
  ...
  bit_len = (string.size + prependlen) << 3
  string += "\x80".force_encoding('ascii-8bit')
  while ((string.size + prependlen) % 64) != 56
    string += "\0"
  end
  ...
  while io.read(64, block)
    x = block.unpack("V16")
    #Debugging
    #puts x.inspect +  "forged a is #{a} and b is #{b}"
    ...
    c = (c + cc) & mask
    d = (d + dd) & mask
    #puts "Printing a debug: " + [a, b, c, d].pack("V4").unpack('H*').join
  end
 
  ret = [a, b, c, d].pack("V4")
  return ret.unpack('H*').join
end
