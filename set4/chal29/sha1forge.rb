...
module SHA1forge 
  ...
  def SHA1forge.circular_shift(bits,word)
    0xFFFFFFFF & (((word) << (bits)) | ((word) >> (32-(bits))))
  end

  def SHA1forge.compress(words, chain)
  ...
  def SHA1forge.hexdigest(message, prependl, iv)
    prependl += message.length
    fill   = "\x00"*(64 - (prependl+9)%64)
    length = "\x00" * 4 + [prependl*8].pack("N*")
